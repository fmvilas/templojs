...
exports.render = function(template, data) {
  ...
  for(attr in template) {
    ...
    if( template_has_key ) {
      ...
      if( attribute.alias && data.hasOwnProperty(attribute.alias) ) {
        key = attribute.alias;
      }

      // We want it to take precedence
      if( data.hasOwnProperty(attr) ) { key = attr; }

      data_has_key = key !== undefined && data.hasOwnProperty(key);

      if( !data_has_key ) {
        if( attribute.required === true ) {
          logError(key, 'Missing required parameter <' + key + '>.');
        } else if( attribute.default !== undefined ) {
          output[key] = getValue(template, attr, key, data);
        }
      } else {
        if( attribute.read_only === true ) {
          logWarning(key, 'Can\'t modify read only parameter <' + key + '>.');
        } else {
          output[key] = getValue(template, attr, key, data);
        }
      }
